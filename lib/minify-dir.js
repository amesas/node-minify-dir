...
function defineDirIO(iPath, oPath) {
  ...
  // Normalize to Unix-like style.
  iPath = iPath.replace(/\\/g, "/");
  oPath = oPath.replace(/\\/g, "/");

  // If last character is '/', remove it so we can add 'mark'.
  {
    let iplc = iPath.charAt(iPath.length - 1);
    let oplc = oPath.charAt(oPath.length - 1);

    if (iplc == "/" || iplc == "\\")
      iPath = iPath.slice(0, -1);
    if (oplc == "/" || oplc == "\\")
      oPath = oPath.slice(0, -1);
  }
